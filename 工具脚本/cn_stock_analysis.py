...
print('=' * 110)
try:
    from chip_cost import analyze_chip_cost, print_chip_report, chip_entry_check
    chip_result = analyze_chip_cost(df, current_price=last_p)
    if chip_result:
        print_chip_report(chip_result, name=f"{name}({code})")
    else:
        print("  数据不足，无法进行筹码分析")
