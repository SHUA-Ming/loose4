...
header = f'{"日期":>12s} {"开盘":>8s} {"收盘":>8s} {"最高":>8s} {"最低":>8s} {"涨幅%":>8s} {"量(手)":>12s} {"额(万)":>10s} {"换手%":>6s} {"MA5":>8s} {"MA10":>8s} {"MA20":>8s}'
print(header)
print('-' * 130)
for _, r in df.iterrows():
    print(f'{r["date"]:>12s} {r["open"]:8.2f} {r["close"]:8.2f} {r["high"]:8.2f} {r["low"]:8.2f} {r["pctChg"]:+8.2f} {r["volume"]:12,.0f} {r["amount_wan"]:10,.0f} {r["turn"]:6.2f} {r["MA5"]:8.2f} {r["MA10"]:8.2f} {r["MA20"]:8.2f}')
...
r20 = df.tail(20)
print(f'近20日均量: {r20["volume"].mean():,.0f}手')
print(f'近5日均量: {df.tail(5)["volume"].mean():,.0f}手')
